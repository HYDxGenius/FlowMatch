...
def get_dataset(args, algorithm, dataset, num_labels, num_classes, data_dir='./data', include_lb_to_ulb=True):
    ...
    """
    from semilearn.datasets import (
        get_cifar, get_stl10,
        get_dtd, get_resisc45, get_clevr, get_retinopathy, get_sun397,
    )
    ...
    if dataset in ["cifar10", "cifar100"]:
        ...
        lb_dset, ulb_dset, eval_dset = get_dtd(args, algorithm, dataset, num_labels, num_classes, data_dir=data_dir, include_lb_to_ulb=include_lb_to_ulb)
    elif dataset == 'resisc45':
        lb_dset, ulb_dset, eval_dset = get_resisc45(args, algorithm, dataset, num_labels, num_classes, data_dir=data_dir, include_lb_to_ulb=include_lb_to_ulb)
    elif dataset == 'clevr':
        lb_dset, ulb_dset, eval_dset = get_clevr(args, algorithm, dataset, num_labels, num_classes, data_dir=data_dir, include_lb_to_ulb=include_lb_to_ulb)
    elif dataset == 'retinopathy':
        lb_dset, ulb_dset, eval_dset = get_retinopathy(args, algorithm, dataset, num_labels, num_classes, data_dir=data_dir, include_lb_to_ulb=include_lb_to_ulb)
    elif dataset == 'sun397':
        lb_dset, ulb_dset, eval_dset = get_sun397(args, algorithm, dataset, num_labels, num_classes, data_dir=data_dir, include_lb_to_ulb=include_lb_to_ulb)
    else:
        raise ValueError(
            f"Dataset {dataset!r} not registered in FlowMatch release. "
            f"Supported: cifar10/cifar100, stl10, dtd, resisc45, clevr, retinopathy, sun397."
        )
    test_dset = None
